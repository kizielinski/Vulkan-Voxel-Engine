...
public:
	...
	//Hierarchy Methods
	void AddChild(NodeTransform* child);
	void RemoveChild(NodeTransform* child);
	void SetChild(NodeTransform* newParent);
	NodeTransform* GetParent();
	NodeTransform* GetChild(unsigned int index);
	int IndexOfChild(NodeTransform* child);
...
private:
	...
	NodeTransform* parent; //Should be null(0) if there is no parent
